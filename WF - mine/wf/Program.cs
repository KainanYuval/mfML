...
namespace wf
{
    static public class Program
    {
        ...
        public static void remoteRun(string configfile)
        {
            ...
            var directories = Directory.GetDirectories(userConfig.dbPath);
            foreach (string dir in directories)
            {
                userConfig.resultsPath = dir + "\\results";
                if (!Directory.Exists(userConfig.resultsPath))
        ...
        static private void RunForDirLoop(String dir)
        {
            //READ DATA
            DB db = new DB();
            db.training_label = db.getDataTable(dir + "\\trainingLabel.txt");
            if (userConfig.setInhyperCube)
                db.training_dt = db.getcleanDb(dir + "\\trainingData.txt", db.training_label.Count());
            else
                db.training_dt = db.getDataTable(dir + "\\trainingData.txt");
            if (userConfig.useCV)
            {
                db.testing_dt = db.training_dt;
                db.testing_label = db.training_label;
            ...
            {
                db.testing_label = db.getDataTable(dir + "\\testingLabel.txt");
                if (userConfig.setInhyperCube)
                    db.testing_dt = db.getcleanDb(dir + "\\testingData.txt", db.testing_label.Count());
                else
                    db.testing_dt = db.getDataTable(dir + "\\testingData.txt");
            }
            db.validation_dt = db.testing_dt;
            db.validation_label = db.testing_label;
